...
//			var yuanselectwidth = $(this).css("width");
//			var yuanselectnumb = $(this).parents().children('select').length;
//			alert(yuanselectwidth);
//			alert(yuanselectnumb);
			...
			var	$thisEl = $(this),
				...
				init = function() {
					...
					{						
						...
						$('#' + uiId).remove();
                        $thisEl.show();
                        $thisEl.parents().css("position","relative");
						$thisEl.css("position","absolute");//特殊的消失方式原来是$thisEl.hide();
						$thisEl.css("left","0");
						$thisEl.css("top","9px");
						$thisEl.css("z-index","-9");
						...
						$thisEl.setOptions = function(){
							...
							var top,
								...
								$optionsInnerEl.parents('.cssSelect').width(widths);
                            if(optionsLimit){
						...
						$thisEl.setOptions();
                        $optionsEl.hide();
                        $thisEl.css("left",$selectEl.position().left);
					}
				},
				...
				events = $.extend({
					...
					selectClick : function() {
						//select 禁用的情况下，禁用重写下拉样式
						if($thisEl.attr("disabled")=="disabled"){
							return false;
						}
						...
						if( $optionsEl.css('display') != 'none' ){
							...
							handlers.selectOn();
							$thisEl.setOptions();
							var yuasukuan=$(this).width();//后来添加的用于修改弹出部分宽度
							$(".optionsBox").width(yuasukuan);//后来添加的用于修改弹出部分宽度
							var qweqweqwe=$(".optionsBox").width();//后来添加的用于修改弹出部分宽度
						}
					},
